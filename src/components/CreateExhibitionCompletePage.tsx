...
export default function CreateExhibitionCompletePage({ 
  ...
  // Use uploaded images or gray placeholders, duplicate for continuous scroll
  const baseImages = uploadedImages.length > 0 
    ? [...uploadedImages.slice(0, 8), ...Array(Math.max(0, 8 - uploadedImages.length)).fill(null)] 
    : Array(8).fill(null);
  
  // Duplicate images for seamless loop
  const displayImagesTop = [...baseImages.slice(0, 4), ...baseImages.slice(0, 4)];
  const displayImagesBottom = [...baseImages.slice(4, 8), ...baseImages.slice(4, 8)];
  ...
  // Calculate total width for continuous scroll
  const imageWidth = 140;
  const gap = 18;
  const totalWidth = (imageWidth + gap) * 4; // 4 images
  ...
  return (
    <div className="bg-white relative w-full min-h-screen max-w-[393px] mx-auto overflow-hidden" data-name="디자인 페이지 생성">
      ...
      <div className="absolute left-1/2 top-[153px] translate-x-[-50%] w-[341px] overflow-hidden">
        ...
        >
          {displayImagesTop.map((img, index) => (
            <div 
              key={`top-${index}`} 
      ...
      <div className="absolute left-1/2 top-[313px] translate-x-[-50%] w-[341px] overflow-hidden">
        ...
        >
          {displayImagesBottom.map((img, index) => (
            <div 
              key={`bottom-${index}`} 
              className="relative shrink-0 size-[140px]"
            >
